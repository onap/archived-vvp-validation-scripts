...
from tests.helpers import load_yaml, prop_iterator, get_param
from tests.structures import Heat
...
ENV_PARAMETER_SPEC = {
    "PLATFORM PROVIDED": [
        ...
        {
            "property": ("vf_module_id",),
            "persistent": False,
            "kwargs": {}
        },
        {
            "property": ("vf_module_index",),
            "persistent": False,
            "kwargs": {}
        },
        {
            "property": ("vf_module_name",),
            "persistent": False,
            "kwargs": {}
        },
        {
            "property": ("workload_context",),
            "persistent": False,
            "kwargs": {}
        },
        {
    ...
    "OS::ContrailV2::InterfaceRouteTable": [
        {
            "property": ("interface_route_table_routes", "interface_route_table_routes_route",),
            "persistent": False,
            "kwargs": {}
        }
    ],
    ...
    "OS::ContrailV2::InstanceIp": [
        {
            ...
            "network_type": "external",
            "kwargs": {
                "exclude_resource": re.compile(r"^.*_int_.*$")
            },
        },
        {
            "property": ("instance_ip_address",),
...
def run_test_parameter(yaml_file, resource_type, *prop, **kwargs):
    ...
    invalid_parameters = []
    param_spec = {}
    parameter_spec = ENV_PARAMETER_SPEC.get(resource_type) # matching spec dict on resource type
    for spec in parameter_spec:
        # iterating through spec dict and trying to match on property
        if spec.get("property") == prop:
            yep = True
            for k, v in kwargs.items():  # now matching on additional kwargs passed in from test (i.e. network_type)
            ...
            if yep:
                ...
                if resource_type == "PLATFORM PROVIDED":
                    if file_is_a_nested_template(yaml_file):
                        pytest.skip("Not checking nested files for PLATFORM PROVIDED params")
                    template_parameters.append({"resource": "", "param": param_spec.get("property")[0]})
                else:
                    all_resources = False
                    if resource_type == "ALL":
                        all_resources = True
                    template_parameters = get_template_parameters(yaml_file, resource_type, param_spec, all_resources=all_resources) # found the correct spec, proceeding w/ test
                break

    for parameter in template_parameters:
        param = parameter.get("param")
        rid = parameter.get("resource")
        ...
        if env_violation(yaml_file, param, spec.get("persistent")):
            ...
            invalid_parameters.append(
                ...
                "parameter {} for {} {} found.".format(resource_type, prop, human_text, param, yaml_file, human_text2)
            )
...
def env_violation(yaml_file, parameter, persistent):
    ...
    if not environment_pair:  # this is a nested file perhaps?
        environment_pair = find_environment_file(yaml_file)  # we want to check parent env
...
def test_neutron_port_external_fixedips_ipaddress_parameter_doesnt_exist_in_environment_file(
    ...
    run_test_parameter(yaml_file, "OS::Neutron::Port", "fixed_ips", "ip_address", network_type="external")
...
def test_neutron_port_internal_fixedips_ipaddress_parameter_exists_in_environment_file(
    ...
    run_test_parameter(yaml_file, "OS::Neutron::Port", "fixed_ips", "ip_address", network_type="internal")
...
def test_neutron_port_fixedips_subnet_parameter_doesnt_exist_in_environment_file(
    ...
    run_test_parameter(yaml_file, "OS::Neutron::Port", "fixed_ips", "subnet", network_type="internal")
...
def test_neutron_port_external_aap_ip_parameter_doesnt_exist_in_environment_file(
    ...
    run_test_parameter(yaml_file, "OS::Neutron::Port", "allowed_address_pairs", "subnet", network_type="external")
...
def test_contrail_route_prefixes_parameter_doesnt_exist_in_environment_file(yaml_file):
    run_test_parameter(yaml_file, "OS::ContrailV2::InterfaceRouteTable", "interface_route_table_routes", "interface_route_table_routes_route")
...
def test_contrail_external_instance_ip_does_not_exist_in_environment_file(yaml_file):
    run_test_parameter(yaml_file, "OS::ContrailV2::InstanceIp", "instance_ip_address", network_type="external")
...
def test_contrail_internal_instance_ip_does_exist_in_environment_file(yaml_file):
    run_test_parameter(yaml_file, "OS::ContrailV2::InstanceIp", "instance_ip_address", network_type="internal")

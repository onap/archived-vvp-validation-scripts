...
def test_network_resource_id_format(yaml_file):
    ...
    for k, v in yml["resources"].items():
        if not is_resource_instanceofdict_properties(v):
            continue
...
def test_network_has_subnet(yaml_file):
    ...
    for k, v in yml["resources"].items():
        if not is_resource_instanceofdict_properties(v) or v.get("type") not in ["OS::Neutron::Net"]:
            continue
    ...
    for k, v in yml["resources"].items():
        if not is_resource_instanceofdict_properties(v) and v.get("type") != "OS::Neutron::Subnet":
            continue
...
def is_resource_instanceofdict_properties(resource):
    """

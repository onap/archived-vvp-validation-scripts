...
from tests.helpers import validates, get_base_template_from_yaml_files, load_yaml
...
def test_internal_network_parameters(yaml_files):
    ...
    for module in incremental_modules:
        ...
        for rid, port in heat.neutron_port_resources.items():
            ...
            param_match = (
                INTERNAL_UUID_PATTERN.match(param) or
                INTERNAL_NAME_PATTERN.match(param)
            )
            if not param_match:
                errors.append((
...
def test_external_network_parameter(heat_template):
    ...
    for rid, port in heat.neutron_port_resources.items():
        ...
        param_match = (
            EXTERNAL_NAME_PATTERN.match(param) or
            EXTERNAL_UUID_PATTERN.match(param)
        )
        if not param_match:
            errors.append((

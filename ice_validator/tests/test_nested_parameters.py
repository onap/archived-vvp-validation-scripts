...
def prop_iterator(resource, *props):
    ...
    props = list(props)
    prop = properties.get(props.pop(0))

    if isinstance(prop, list):
        for x in prop:
            yield nested_dict.get(x, *props, "get_param")
    elif isinstance(prop, dict):
        yield nested_dict.get(prop, *props, "get_param")
...
# There isn't a parameter that corresponds to this property
# @validates("R-708564")
# def test_vmi_aap_parameter_name_doesnt_change_in_nested_template(yaml_file):
#    check_nested_parameter_doesnt_change(yaml_file, "OS::ContrailV2::VirtualMachineInterface", "virtual_machine_interface_allowed_address_pairs")

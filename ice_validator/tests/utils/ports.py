...
from tests.helpers import parameter_type_to_heat_type, get_defined_parameter_type
...
def check_ip_format(yaml_file, regx, port_type, resource_property, nested_property):
    ...
    for rid, resource in ports.items():
        ...
        for param in params:
            ...
            parameter, parameter_type = parameter_type_to_heat_type(parameter)

            # checking parameter format = type defined in template
            heat_parameter_type = get_defined_parameter_type(heat_parameters, parameter)

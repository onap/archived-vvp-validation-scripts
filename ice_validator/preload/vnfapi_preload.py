...
class VnfApiPreload:
    ...
    def preload(self):
        ...
        for vm in self.vms:
            preload["input"]["vnf-topology-information"]["vnf-assignments"][
                "vnf-vms"
            ].append(vm)
    ...
    def add_availability_zones(self, new_zone):
        availability_zones = self._preload["input"]["vnf-topology-information"][
            "vnf-assignments"
        ]["availability-zones"]
        ...
        for zone in availability_zones:
            ...
            if new_zone == z:
                continue
            self._preload["input"]["vnf-topology-information"]["vnf-assignments"][
                "availability-zones"
            ].append({"availability-zone": new_zone})

    def add_vnf_network(self, network_role, network_name=""):
        networks = self._preload["input"]["vnf-topology-information"][
            "vnf-assignments"
        ]["vnf-networks"]
    ...
    def _add_vm_ips(self, vm_type, network_role, version, ips=None):
        ips = ips or []

...
class GrApiPreload:
    ...
    def preload(self):
        ...
        for vm in self.vms:
            preload["input"]["preload-vf-module-topology-information"]["vf-module-topology"]["vf-module-assignments"]["vms"]["vm"].append(vm)

        for param in self.parameters:
            preload["input"]["preload-vf-module-topology-information"]["vf-module-topology"]["vf-module-parameters"]["param"].append(param)

        return preload

    def init_preload(self):
        self._preload = get_json_template("preload_template")
        self._preload["input"]["preload-vf-module-topology-information"]["vnf-topology-identifier-structure"]["vnf-name"] = self.vnf_name
        self._preload["input"]["preload-vf-module-topology-information"]["vnf-topology-identifier-structure"]["vnf-type"] = self.vnf_type

    def add_availability_zones(self, *zones):
        availability_zones = self._preload["input"]["preload-vf-module-topology-information"]["vnf-resource-assignments"]["availability-zones"]["availability-zone"]

        for zone in zones:
            if zone in availability_zones:
                continue
            self._preload["input"]["preload-vf-module-topology-information"]["vnf-resource-assignments"]["availability-zones"]["availability-zone"].append(zone)

    def add_vnf_network(self, network_role, network_name=""):
        networks = self._preload["input"]["preload-vf-module-topology-information"]["vnf-resource-assignments"]["vnf-networks"]["vnf-network"]
        ...
        self._preload["input"]["preload-vf-module-topology-information"]["vnf-resource-assignments"]["vnf-networks"]["vnf-network"].append(jdata)
    ...
    def _add_vm_ips(self, vm_type, network_role, version, ips=[]):
        ...
        count = network_obj["network-information-items"]["network-information-item"][idx]["ip-count"]
        for ip in ips:
            network_obj["network-information-items"]["network-information-item"][idx]["network-ips"]["network-ip"].append(ip)
            count += 1

        network_obj["network-information-items"]["network-information-item"][idx]["ip-count"] = count
